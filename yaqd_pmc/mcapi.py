...
MC_CAPTURE_TORQUE = (
    128  # Specifies captured torque data for GetCaptureData( ) and GetAxisConfiguration( ).
)
...
MC_COUNT_CAPTURE = (
    1  # GetCount( ) should retrieve the number of captured positions from high-speed capture mode.
)
...
MC_COUNT_FILTER = (
    8  # GetCount( ) should retrieve the number of digital filter coefficients currently loaded.
)
MC_COUNT_FILTERMAX = (
    16  # GetCount( ) should retrieve the maximum number of digital filter coefficients supported.
)
...
MC_DIO_FIXED = (
    256  # Channel is a fixed input or output and cannot be changed using ConfigureDigitalIO( ).
)
...
MC_DIO_STEPPER = (
    1024  # Input channel has been dedicated to driving a stepper motor (DC2-PC or DC2-STN).
)
...
MC_ENC_FAULT_PRI = (
    1  # Enables encoder fault detection for the primary encoder with EnableEncoderFault( ).
)
MC_ENC_FAULT_AUX = (
    2  # Enables encoder fault detection for the auxiliary encoder with EnableEncoderFault( ).
)
...
MC_LIMIT_BOTH = (
    3  # Enables both the positive and negative limits, used with GetLimits( ) / SetLimits( ).
)
...
MC_LRN_POSITION = (
    1  # Indicates LearnPoint( ) should learn the current actual position for the specified axis.
)
MC_LRN_TARGET = (
    2  # Indicates LearnPoint( ) should learns the current target position for the specified axis.
)
...
MC_OM_PIEZO = (
    4  # Servo axis set to/in piezo operation, see GetModuleOutputMode( ) / SetModuleOutputMode( ).
)
...
MC_OPEN_EXCLUSIVE = (
    0x8000  # Combine with MC_OPEN_ASCII or MCOPEN_BINARY for exclusive access to interface.
)
...
MC_PROF_TRAPEZOID = (
    1  # Selects/indicates a trapezoidal accel/decel profile for GetProfile( ) / SetProfile( ).
)
MC_PROF_SCURVE = (
    2  # Selects/indicates that an S-curve accel/decel profile for GetProfile( ) / SetProfile( ).
)
MC_PROF_PARABOLIC = (
    4  # Selects/indicates that a parabolic accel/decel profile for GetProfile( ) / SetProfile( ).
)
...
MC_STEP_FULL = (
    1  # Selects full step stepper motor operation in GetMotionConfigEx( ) / SetMotionConfigEx( )
)
MC_STEP_HALF = (
    2  # Selects half step stepper motor operation in GetMotionConfigEx( ) / SetMotionConfigEx( )
)
...
MC_TYPE_REG = (
    1  # Specifies data in controller register for functions that accept varaible data types.
)
MC_TYPE_LONG = (
    2  # Specifies long integer (32-bit) data type for functions that accept varaible data types.
)
MC_TYPE_FLOAT = (
    3  # Specifies floating point (32-bit) data type for functions that accept varaible data types.
)
...
MCERR_MODE_UNAVAIL = (
    4  # Error code:  controller already open in different mode, MCERRMASK_SYSTEM group.
)
MCERR_UNSUPPORTED_MODE = (
    5  # Error code:  controller doesn't support this mode, MCERRMASK_SYSTEM group.
)
MCERR_INIT_DRIVER = (
    6  # Error code:  couldn't initialize the device driver, MCERRMASK_SYSTEM group.
)
...
MCERR_OS_ERROR = (
    9  # Error code:  operating sytem function reported an error, MCERRMASK_SYSTEM group.
)
MCERR_NOTSUPPORTED = (
    11  # Error code:  controller doesn't support this feature, MCERRMASK_UNSUPPORTED group.
)
...
class Mcapi:
    ...
    def __init__(self):
        ...
        self._dll.MCArcEndAngle.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.c_short,
            ctypes.c_double,
        ...
        self._dll.MCCaptureData.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.c_int,
            ctypes.c_double,
        ...
        self._dll.MCConfigureCompare.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
        ...
        self._dll.MCEnableBacklash.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.c_double,
            ctypes.c_short,
        ...
        self._dll.MCEnableGearing.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.c_ushort,
            ctypes.c_double,
        ...
        self._dll.MCGetAccelerationEx.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(ctypes.c_double),
        ]
        self._dll.MCGetAnalogEx.argtypes = [HCTRLR, ctypes.c_ushort, ctypes.POINTER(ctypes.c_uint)]
        self._dll.MCGetAuxEncIdxEx.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(ctypes.c_double),
        ]
        self._dll.MCGetAuxEncPosEx.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(ctypes.c_double),
        ]
        self._dll.MCGetAxisConfiguration.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(MCAXISCONFIG),
        ]
        self._dll.MCGetBreakpointEx.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(ctypes.c_double),
        ]
        self._dll.MCGetCaptureData.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.c_int,
            ctypes.c_int,
        ...
        self._dll.MCGetCaptureSettings.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
        ...
        self._dll.MCGetContourConfig.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(MCCONTOUR),
        ]
        self._dll.MCGetContourConfig.restype = ctypes.c_short
        self._dll.MCGetCount.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.c_int,
            ctypes.POINTER(ctypes.c_int),
        ]
        self._dll.MCGetDecelerationEx.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(ctypes.c_double),
        ]
        self._dll.MCGetDigitalFilter.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(ctypes.c_double),
            ctypes.c_int,
        ...
        self._dll.MCGetDigitalIOConfig.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(ctypes.c_ushort),
        ]
        self._dll.MCGetDigitalIOEx.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
        ...
        self._dll.MCGetFilterConfigEx.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(MCFILTEREX),
        ]
        self._dll.MCGetFollowingError.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(ctypes.c_double),
        ]
        self._dll.MCGetGain.argtypes = [HCTRLR, ctypes.c_ushort, ctypes.POINTER(ctypes.c_double)]
        self._dll.MCGetIndexEx.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(ctypes.c_double),
        ]
        self._dll.MCGetInstalledModules.argtypes = [
            HCTRLR,
            ctypes.POINTER(ctypes.c_long),
            ctypes.c_int,
        ]
        self._dll.MCGetJogConfigEx.argtypes = [HCTRLR, ctypes.c_ushort, ctypes.POINTER(MCJOGEX)]
        self._dll.MCGetLimits.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
        ...
        self._dll.MCGetModuleInputMode.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(ctypes.c_int),
        ]
        self._dll.MCGetModuleOutputMode.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(ctypes.c_ushort),
        ]
        self._dll.MCGetMotionConfigEx.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(MCMOTIONEX),
        ]
        self._dll.MCGetOperatingMode.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(ctypes.c_int),
        ]
        self._dll.MCGetOptimalEx.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(ctypes.c_double),
        ]
        self._dll.MCGetPositionEx.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(ctypes.c_double),
        ]
        self._dll.MCGetProfile.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(ctypes.c_ushort),
        ]
        ...
        self._dll.MCGetServoOutputPhase.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(ctypes.c_ushort),
        ]
        ...
        self._dll.MCGetTargetEx.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(ctypes.c_double),
        ]
        ...
        self._dll.MCGetVectorVelocity.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(ctypes.c_double),
        ]
        self._dll.MCGetVelocityActual.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(ctypes.c_double),
        ]
        self._dll.MCGetVelocityEx.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(ctypes.c_double),
        ]
        self._dll.MCGetVelocityOverride.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(ctypes.c_double),
        ]
        ...
        self._dll.MCInterruptOnPosition.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.c_int,
            ctypes.c_double,
        ...
        self._dll.MCSetCommutation.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(MCCOMMUTATION),
        ]
        self._dll.MCSetContourConfig.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(MCCONTOUR),
        ]
        ...
        self._dll.MCSetDigitalFilter.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(ctypes.c_double),
            ctypes.c_int,
        ]
        self._dll.MCSetFilterConfigEx.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(MCFILTEREX),
        ]
        ...
        self._dll.MCSetLimits.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.c_short,
            ctypes.c_short,
        ...
        self._dll.MCSetMotionConfigEx.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.POINTER(MCMOTIONEX),
        ]
        self._dll.MCSetOperatingMode.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.c_ushort,
            ctypes.c_ushort,
        ...
        self._dll.MCSetTimeoutEx.argtypes = [
            HCTRLR,
            ctypes.c_double,
            ctypes.POINTER(ctypes.c_double),
        ]
        ...
        self._dll.pmccmdrpyex.argtypes = [
            HCTRLR,
            ctypes.c_ushort,
            ctypes.c_ushort,
            ctypes.c_void_p,
        ...
        self._dll.pmclookupvar.argtypes = [
            HCTRLR,
            ctypes.c_uint,
            ctypes.c_char_p,
            ctypes.POINTER(ctypes.c_uint),
    ...
    def GetCaptureData(self, axis, type, start, points):
        ...
        self.ProcessException(
            self._dll.MCGetCaptureData(
                self._handle,
                axis,
                type,
                start,
                points,
                ctypes.cast(temp, ctypes.POINTER(ctypes.c_double)),
            )
        )
    ...
    def GetCaptureSettings(self, axis):
        ...
        index = ctypes.c_int()
        self.ProcessException(
            self._dll.MCGetCaptureSettings(self._handle, axis, points, period, delay, index)
        )
    ...
    def GetInstalledModules(self, size=16):
        ...
        self.ProcessException(
            self._dll.MCGetInstalledModules(
                self._handle, ctypes.cast(temp, ctypes.POINTER(ctypes.c_long)), size
            )
        )
        # convert data to a friendly python list
        modules = []
    ...
    def GetLimits(self, axis):
        ...
        self.ProcessException(
            self._dll.MCGetLimits(
                self._handle, axis, hardlimitmode, softlimitmode, softlimitminus, softlimitplus
            )
        )
        return (
            hardlimitmode.value,
            softlimitmode.value,
            softlimitminus.value,
            softlimitplus.value,
        )
    ...
    def pmccmdex(self, axis, command, argument, type):
        ...
        if type == MC_TYPE_NONE:
            ...
            raise McapiException(self.TranslateErrorEx(MCERR_CONSTANT))
        self.ProcessException(
            self._dll.pmccmdex(self._handle, axis, command, ctypes.byref(arg), type)
        )
    ...
    def pmcgetramex(self, offset, buffer, size):
        ...
        self.ProcessException(
            self._dll.pmcgetramex(self._handle, offset, ctypes.byref(buffer), size)
        )
    ...
    def pmcputramex(self, offset, buffer, size):
        ...
        self.ProcessException(
            self._dll.pmcputramex(self._handle, offset, ctypes.byref(buffer), size)
        )
